...
def tweets_per_minute_users(df, indexes, key_words):
    ...
    :param indexes: indexes generated by get _users_indexes
    :param key_words: keywords to look for
    :return: return a dataframe with the words frequency but for username filter
    '''
    index_time = pd.Index(df['dateTweet'].sort_values().unique())
    politicosdf = df.iloc[indexes].filter(['tweet', 'dateTweet'])
    politicosdf['All'] = 1
